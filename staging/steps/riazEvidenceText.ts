...
const Q3_USER_CHARTS: UserChartPlan[] = [
  ...
  {
    key: 'means', templateId: 'bar_grouped_v1', dataset: 'PAIRED', filters: [inList('gene', IFNG_FOCUS), eq('response', 'R')], bindings: { x: 'gene', y: 'log2_cpm', color: 'timepoint' }, title: 'Q3 · IFN-γ panel Pre vs On means, responders (PAIRED)',
...
const Q5_USER_CHARTS: UserChartPlan[] = [
  ...
  { key: 'onLevel', templateId: 'bar_grouped_v1', dataset: 'PAIRED', filters: [inList('gene', CYTOTOXIC_FOCUS), eq('timepoint', 'On')], bindings: { x: 'gene', y: 'log2_cpm', color: 'prior_ipi' }, title: 'Q5 · Cytotoxic panel on-treatment level by stratum (PAIRED)', reading: 'On-treatment mean level per gene, split by prior ipilimumab exposure — the induction difference collapsed to a single timepoint.' },
  {
...
const Q10_USER_CHARTS: UserChartPlan[] = [
  ...
  {
    key: 'means', templateId: 'bar_grouped_v1', dataset: 'PAIRED', filters: [inList('gene', APM_FOCUS)], bindings: { x: 'gene', y: 'log2_cpm', color: 'timepoint' }, title: 'Q10 · Antigen-presentation panel Pre vs On means (PAIRED)',

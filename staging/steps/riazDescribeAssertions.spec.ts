...
import { citedConnectorOf, decisionOf, filterText, nGroupsOf } from './riazDescribeObserve';
...
function observedQ12(overrides: Partial<ObservedDescribeResult> = {}): ObservedDescribeResult {
  ...
  return {
    ...
    boundParameters: { groupColumns: ['gene'], valueColumn: 'log2_cpm', aggregation: 'mean', direction: 'desc' },
...
test('UT-E2E-DESC-007: the Q12 sentence is compared byte-for-byte', () => {
  const almost = Q12_SENTENCE.replace('; IFNG the lowest', ', IFNG the lowest');
  assert.ok(namesOfFailures(evaluateDescribeResult(Q12, observedQ12({ sentence: almost, decision: { id: 'd', type: 'descriptive_summary', ruleRunId: 'run-1', sentenceText: almost, status: 'draft' } }))).includes('rendered sentence (exact)'));
});

test('UT-E2E-DESC-008: a decision of another type, another run, or another sentence fails', () => {
  ...
  const wrongRun = observedQ12({ decision: { id: 'd', type: 'descriptive_summary', ruleRunId: 'run-9', sentenceText: Q12_SENTENCE, status: 'draft' } });
  assert.ok(namesOfFailures(evaluateDescribeResult(Q12, wrongRun)).includes('decision context.ruleRunId'));
  ...
  assert.ok(namesOfFailures(evaluateDescribeResult(Q12, drifted)).includes('decision sentence === rendered sentence'));
});

test('UT-E2E-DESC-009: an unbound or overridden connector parameter fails, naming the parameter', () => {
  const unbound = observedQ12({ boundParameters: { groupColumns: ['gene'], valueColumn: 'log2_cpm', direction: 'desc' } });
...
test('UT-E2E-DESC-010: EC6 — a Q14 count sentence that says "rows" instead of "patients" fails', () => {
  ...
  const base: ObservedDescribeResult = {
    ...
    boundParameters: { groupColumns: ['response', 'prior_ipi'], distinctKey: 'patient_id' },
    rows,
    nGroups: 4,

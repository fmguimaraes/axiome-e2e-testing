...
export const QUESTIONS: Question[] = [
  ...
  // ── AXI-1581: the four Phase-1 shape connectors (epic AXI-1575) ──
  { id: 'Q32', dataset: 'PAIRED', text: 'At the pre-treatment biopsy, what is the median log2 CPM of each of the 24 immune panel genes, ranked from highest to lowest? Plan exactly two nodes: one filter node with the single condition timepoint = Pre, then one describe node depending on it that cites the connector rule SUM-RANK-MEDIAN-01 with groupColumns = [gene], valueColumn = log2_cpm, aggregation = median and direction = desc.' },
  { id: 'Q33', dataset: 'PAIRED', text: 'At the pre-treatment biopsy, which immune panel genes vary the most across patients (standard deviation of log2 CPM, most variable first)? Plan exactly two nodes: one filter node with the single condition timepoint = Pre, then one describe node depending on it that cites the connector rule SUM-SPREAD-01 with groupColumns = [gene], valueColumn = log2_cpm, aggregation = std and direction = desc.' },
  { id: 'Q34', dataset: 'PAIRED', text: 'At the pre-treatment biopsy, what is the highest log2 CPM reached by each of the 24 immune panel genes, ranked from highest to lowest? Plan exactly two nodes: one filter node with the single condition timepoint = Pre, then one describe node depending on it that cites the connector rule SUM-EXTREMES-01 with groupColumns = [gene], valueColumn = log2_cpm, aggregation = max and direction = desc.' },
  { id: 'Q35', dataset: 'PAIRED', text: 'How many patients are in each response × timepoint group? Plan exactly two nodes: ONE filter node with the single condition gene = CD8A (that single-gene slice makes one row one patient per timepoint), then one describe node depending on it that cites the connector rule SUM-CROSS-COUNT-01 with groupColumns = [response, timepoint] and distinctKey = patient_id, so the counts are distinct patients and not rows.' },

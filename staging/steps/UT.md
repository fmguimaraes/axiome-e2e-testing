...
`stage:rules` over the describe surface: the ten SUMMARY **connector** rules
...
will bind. `connectorShapes.ts` mirrors the ten seeded parameter schemes just
...
| ID | Description | Status |
|----|-------------|--------|
| UT-E2E-CONN-1581-001 | every expected describe result cites a connector that can bind its own declared parameters (operation, arity, enums, pinned-absent params) | Pass |
...
| UT-E2E-CONN-1581-005 | the guard separates the two top-N connectors by the presence of a filter — SUM-TOPN-01 is pinned to an absent one, SUM-TOPN-FILTERED-01 requires one (AXI-1582) | Pass |
